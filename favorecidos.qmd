...
favorecidos_2023_01 %>% filter(tipo_favorecido == "PF", str_detect(codigo_favorecido,"470.444-") ) %>% group_by(nome_codigo_favorecido) %>% count()  %>% head() %>% kable()
```

Depois da visão geral e dos pequenhos ajustes nós podemos analisar melhor os detalhes sobre os pagadores e sobre os recebedores.
...
favorecidos_amostra %>% group_by( nome_unidade_gestora, codigo_unidade_gestora) %>% head(10) %>% count() %>% arrange(desc(n))  %>% kable()
```
