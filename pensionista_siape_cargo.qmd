...
### Frequência tempo de contribuição

```{r}

df_plot_bell <- tabela_pensionista_siape %>% filter(tempo_setor_publico>0)  %>% group_by(inicio_servico_publico,inicio_pensao)  %>% summarise(valor = sum(remuneracao_basica_bruta_r), n=n()) %>% drop_na()
