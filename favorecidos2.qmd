...
# lista_ugs <- read_excel("favorecidos/lista_ugs.xlsx") %>% clean_names()


municipios <- read_csv("municipios.csv")

# municipios <-  municipios %>% mutate(municipio = as.character(municipio))
...
# municipios <- left_join(municipios %>%  select(municipio, muni_uf, lat, lon), lista_ugs %>% select(ug_executora_codigo, uge_municipio_codigo_ibge), by = c( "municipio"= "uge_municipio_codigo_ibge"))
# 
# favorecidos_2023_01 <-  favorecidos_2023_01 %>% mutate(codigo_unidade_gestora = as.character(codigo_unidade_gestora))
...
um_pagador_varios_favorecidos <- left_join(  favorecidos_2023_01%>% filter(tipo_favorecido == "PJ", muni_uf_pagadora == local_cidade) %>% group_by(muni_uf_favorecido, muni_uf_pagadora) %>% summarise(valor_recebido = sum(valor_recebido)) , municipios %>% select(lon,lat, muni_uf_favorecido))


varios_pagadores_um_favorecido <- left_join(  favorecidos_2023_01%>% filter(tipo_favorecido == "PJ", muni_uf_favorecido == local_cidade) %>% group_by(muni_uf_pagadora, muni_uf_favorecido) %>% summarise(valor_recebido = sum(valor_recebido)) , municipios %>% select(lon,lat, muni_uf_pagadora))
