...
receita %>% filter(valor_realizado<0, ano_exercicio == 2022) %>% group_by(nome_orgao_superior) %>% summarise(valores_negativos = sum(valor_realizado)/1000000000) %>% arrange(valores_negativos) %>% head()%>% kable( col.na = c("Órgão Superior","R$ bi"),digits = 1, format.args = list(big.mark = ".", decimal.mark = ",") )
```
