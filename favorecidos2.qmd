...
um_pagador_varios_favorecidos <- left_join(  favorecidos_2023_01%>% filter(tipo_favorecido == "PJ", muni_uf_pagadora == local_cidade) %>% group_by(muni_uf_favorecido, municipio_ug_pagadora) %>% summarise(valor_recebido = sum(valor_recebido)) , municipios)

#recife <- left_join(recife, all_muni,  by = c("municipio" = "code_muni"))


varios_pagadores_um_favorecido <- left_join(  favorecidos_2023_01%>% filter(tipo_favorecido == "PJ", muni_uf_favorecido == local_cidade) %>% group_by(muni_uf_pagadora, municipio_favorecido) %>% summarise(valor_recebido = sum(valor_recebido)) , municipios)
...
leaflet() %>% addTiles() %>%
  addCircles( data = varios_pagadores_um_favorecido, color = "blue", weight = 1,
    radius = ~sqrt(valor_recebido) * 30, popup = ~ paste0("Pago ",Format( valor_recebido, fmt=myNumFmt), " para: ", municipio_favorecido
  ))%>% 
  leaflet::addCircles (data=um_pagador_varios_favorecidos, color = "red", weight = 1,
    radius = ~sqrt(valor_recebido) * 30, popup = ~ paste0("Pago ",Format( valor_recebido, fmt=myNumFmt), " para: ", muni_uf_favorecido
  ))
```
