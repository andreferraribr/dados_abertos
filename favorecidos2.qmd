...
municipios <- municipios %>% mutate (muni_uf_favorecido = paste0(str_to_upper(name)," - ", uf_code) )

municipios <- municipios %>% mutate(muni_uf_pagadora = paste0( str_to_upper(no_accents), " - ",uf_code))

favorecidos_2023_01 <- favorecidos_2023_01 %>% mutate (muni_uf_favorecido = paste0( municipio_favorecido," - ", uf_favorecido) )

favorecidos_2023_01 <- favorecidos_2023_01 %>% mutate (muni_uf_pagadora = paste0( municipio_ug_pagadora," - ", uf_ug_pagadora) )
```
...
local_cidade <- "CAMPINAS - SP"

um_pagador_varios_favorecidos <- left_join(  favorecidos_2023_01%>% filter(tipo_favorecido == "PJ", muni_uf_pagadora == local_cidade) %>% group_by(muni_uf_favorecido) %>% summarise(valor_recebido = sum(valor_recebido)) , municipios)

#recife <- left_join(recife, all_muni,  by = c("municipio" = "code_muni"))


varios_pagadores_um_favorecido <- left_join(  favorecidos_2023_01%>% filter(tipo_favorecido == "PJ", muni_uf_favorecido == local_cidade) %>% group_by(muni_uf_pagadora) %>% summarise(valor_recebido = sum(valor_recebido)) , municipios)
...
#| error: false
#| layout-ncol: 2
#| label: tbl-origem_destino
#| tbl-cap: Origem e destino dos valores (R$).
#| tbl-subcap: 
#|    - "Um pagador e vários favorecidos"
#|    - "Vários pagadores e um favorecido"



favorecidos_2023_01%>% filter(tipo_favorecido == "PJ", muni_uf_pagadora == local_cidade) %>% group_by( municipio_ug_pagadora, muni_uf_favorecido)%>% summarise(valor_recebido = sum(valor_recebido))%>% arrange(desc(valor_recebido)) %>% head(10) %>% kable(col.names = c("Município UG Pagadora", "Município do Favorecido", "Valor Recebido (R$)"), digits = 0, format.args = list(big.mark = ".", decimal.mark = ","))%>%
  kable_styling(font_size = 9)

favorecidos_2023_01%>% filter(tipo_favorecido == "PJ", muni_uf_favorecido == local_cidade) %>% group_by(muni_uf_pagadora, municipio_favorecido) %>% summarise(valor_recebido = sum(valor_recebido)) %>% arrange(desc(valor_recebido))%>% head(10) %>% kable(col.names = c("Município UG Pagadora", "Município do Favorecido", "Valor Recebido (R$)"), digits = 0, format.args = list(big.mark = ".", decimal.mark = ","))%>%
  kable_styling(font_size = 9)
...
#| fig-cap: Origem e destino dos valores (R$).
#| fig-subcap:
#|    - "Um pagador e vários favorecidos"
#|    - "Vários pagadores e um favorecido"

leaflet(um_pagador_varios_favorecidos) %>% addTiles() %>%
  addCircles( color = "red", weight = 1,
    radius = ~sqrt(valor_recebido) * 30, popup = ~ muni_uf_favorecido
  )

leaflet(varios_pagadores_um_favorecido) %>% addTiles() %>%
  addCircles(color = "blue", weight = 1,
    radius = ~sqrt(valor_recebido) * 30, popup = ~ muni_uf_favorecido
  )
...
leaflet() %>% addTiles() %>%
  addCircles( data = varios_pagadores_um_favorecido, color = "blue", weight = 1,
    radius = ~sqrt(valor_recebido) * 30, popup = ~ muni_uf_favorecido
  )%>% 
  leaflet::addCircles (data=um_pagador_varios_favorecidos, color = "red", weight = 1,
    radius = ~sqrt(valor_recebido) * 30, popup = ~ paste0("Pago ", valor_recebido, "para: ", muni_uf_favorecido
  ))
```
