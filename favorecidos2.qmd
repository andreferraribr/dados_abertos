...
varios_pagadores_um_favorecido <- left_join(  favorecidos_2023_01%>% filter(tipo_favorecido == "PJ", muni_uf_favorecido == local_cidade) %>% group_by(muni_uf_pagadora, muni_uf_favorecido) %>% summarise(valor_recebido = sum(valor_recebido)) , municipios %>% select(lon,lat, muni_uf), by = c( "muni_uf_pagadora" ="muni_uf"))
...
A base de dados do Siafi não contempla caractéres com acentos e cedilha. Por este motivo, foi necessário mesclar várias bases de dados para obter o nome correto das localidade (SÃO PAULO x SAO PAULO).
